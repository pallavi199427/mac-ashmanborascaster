...
MULTICAST_OUTPUT="udp://${MULTICAST_IP}:${MULTICAST_PORT}?pkt_size=${MULTICAST_PKT_SIZE}&ttl=${MULTICAST_TTL}&localaddr=127.0.0.1"
...
write_metrics_snapshot() {
  ...
  # Build detected format label (e.g. "1080i50" or "1080p25")
  local _det_fmt="${FORMAT_CODE}"
  if [[ -n "${DETECTED_WIDTH:-}" && -n "${DETECTED_HEIGHT:-}" && -n "${DETECTED_VF_FPS:-}" ]]; then
    local _scan_suffix="p"
    [[ "${DETECTED_SCAN:-}" == "interlaced" ]] && _scan_suffix="i"
    _det_fmt="${DETECTED_HEIGHT}${_scan_suffix}${DETECTED_VF_FPS}"
  fi

  atomic_write "${METRICS_JSON}" <<EOF
{"ts":"${t}","service":"yt-ingest","uptime_s":${uptime},"mode":"$(json_escape "${MODE}")","state":"$(json_escape "${state}")","sdi_signal":${sdi_sig},"device":"$(json_escape "${DECKLINK_DEVICE}")","format":"$(json_escape "${_det_fmt}")","multicast":"${MULTICAST_IP}:${MULTICAST_PORT}","ffmpeg":${ff_detail},"switching":{"ok_count":${OK_COUNT},"bad_count":${BAD_COUNT},"probe_ok":${PROBE_OK_COUNT},"probe_bad":${PROBE_BAD_COUNT}},"restarts":{"consecutive_failures":${CONSEC_FAILS},"backoff_s":${BACKOFF},"total_mode_starts":${TOTAL_MODE_STARTS},"total_ffmpeg_exits":${TOTAL_FFMPEG_EXITS},"last_exit_rc":${LAST_EXIT_RC}}}
EOF
}
...
build_ffmpeg_live_cmd() {
  ...
  local gop="${DETECTED_GOP:-${GOP}}"
  local keyint="${DETECTED_GOP:-${KEYINT_MIN}}"
